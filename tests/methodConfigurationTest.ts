...
import {MethodConfiguration} from "../src/methodConfiguration";
...
describe("MethodConfiguration", () => {
	...
	describe("isSuitable", () => {
		...
		it("must be false if args count differs", () => {
			const configuration = new MethodConfiguration<void>("test", [1]);

			expect(configuration.isSuitable([])).to.be(false);
		});

		it("must be true if args match", () => {
			const configuration = new MethodConfiguration<void>("test", [1, 2, 3, 4]);
		...
		it("must match constraints", () => {
			...
			const configuration = new MethodConfiguration<void>("test", args);
	...
	describe("returns", () => {
		...
		it("must return passed value", () => {
			const configuration = new MethodConfiguration<number>("test", []);
		...
		it("must not allow multiple configurations", () => {
			...
			expect(() => configuration.returns(callback)).to.throwError(/Method test1 call is already configured/);
			expect(() => configuration.throws(new Error("Test error"))).to.throwError(/Method test1 call is already configured/);
		});
	...
	describe("throws", () => {
		...
		it("must not allow multiple configurations", () => {
			...
			expect(() => configuration.throws(new Error("Test error"))).to.throwError(/Method test1 call is already configured/);
			expect(() => configuration.returns([])).to.throwError(/Method test1 call is already configured/);
		});
	...
	describe("onCall", () => {
		...
		it("must set call number only once", () => {
			...
			expect(() => configuration.onCall(1)).to.throwError(/Method test1 call number is already configured/);
		});
